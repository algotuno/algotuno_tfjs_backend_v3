const {tidy, tensor2d, sequential, layers, train} = require( "@tensorflow/tfjs-node");
const {next_day} = require( "./date_util.js");
...
function get_stock_data(_stock_data, _trading_days_for_the_year, _days_range, _days_range_multiplier) {
    ...
    let tensor_validation_xs_list = tidy(() => tensor2d(prep_tensor_validation_xs_list));
    let tensor_validation_ys_list = tidy(() => tensor2d(prep_tensor_validation_ys_list));
    // testing dataset
    ...
    let tensor_testing_xs_list = tidy(() => tensor2d(prep_tensor_testing_xs_list));
    let tensor_testing_ys_list = tidy(() => tensor2d(prep_tensor_testing_ys_list));
    // testing dataset
    ...
    let tensor_training_xs_list = tidy(() => tensor2d(prep_tensor_training_xs_list));
    let tensor_training_ys_list = tidy(() => tensor2d(prep_tensor_training_ys_list));
...
const nn_model = (training_data, labelled_data) => tidy(() => {
    /**
     *
    ...
    const ALPHA = 0.00001;
    model.compile({optimizer: train.adam(ALPHA), loss: "meanSquaredError"});
    return model;
});
...
async function forecast(_epoch, _model, _testing_raw_xs_list, _testing_raw_ys_list) {
    function forecast_curr(_raw_xs_list, _raw_ys_list) {
        ...
        let tensor_forecast_xs_list = tidy(() => tensor2d(prep_tensor_forecast_xs_list));
        let tensor_forecast_ys_list = tidy(() => tensor2d(prep_tensor_forecast_ys_list));
    ...
    for (let i = 1; i <= _total_runs; i++) {
        ...
        if (i === 1) {
            ...
            const forecast_on_consolidated_xs_list = tidy(
                () => tensor2d(prep_tensor_forecast_xs_list)
            );
